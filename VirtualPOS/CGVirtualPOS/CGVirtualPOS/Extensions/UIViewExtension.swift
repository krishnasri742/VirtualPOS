...
extension UITableView {
    ...
    func reloadAndAnimateTable(table:UITableView) {
        table.reloadData()
...
extension String {
    ...
    func toPhoneNumber() -> String {
        return self.replacingOccurrences(of: "(\\d{1})(\\d{3})(\\d{3})(\\d{3})", with: "$1-$2-$3-$4", options: .regularExpression, range: nil)
    }
}
...
extension UIColor {
    static func colorWithHexString (_ hex:String) -> UIColor {
        ...
        if (cString.count != 6) {
            return UIColor.fgApplicationGrayColor()
        }
    ...
    static func fgApplicationGrayColor() -> UIColor {
        return colorWithHexString("#BDBDBD")
    }
}
...
extension UIView {
    ...
    func addGradientButtonSublayer(topColor:UIColor,bottomColor:UIColor,btn:UIButton) -> CAGradientLayer{
        let gl = CAGradientLayer()
        gl.colors = [topColor.cgColor, bottomColor.cgColor] as [AnyObject]
        //gl.frame = btn.bounds
        gl.startPoint = CGPoint(x: 0, y: 1.0)
        gl.endPoint = CGPoint(x: 1.0, y: 0.5)
        //self.layer.insertSublayer(gl, at: 0)
        return gl
    }
...
extension UIView {
     ...
     */
    func bottomY() -> CGFloat { return top() + heightt }
    ...
    func centerY(_ constant: CGFloat = 0) -> UIView {
        if translatesAutoresizingMaskIntoConstraints {
            center = CGPoint(x: superview!.center.x, y: CGFloat(superview!.heightt/2) + constant)
        } else {
